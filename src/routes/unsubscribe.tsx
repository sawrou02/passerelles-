...
export const Route = createFileRoute("/unsubscribe")({
  head: () => ({
    meta: [{ title: "Désabonnement — MyKutub" }, { name: "robots", content: "noindex" }],
  }),
  component: Page,
});

function Page() {
  const [status, setStatus] = useState<"idle" | "loading" | "done" | "need_login" | "error">(
    "idle",
  );
  ...
  return (
    <div className="min-h-[60vh] flex items-center justify-center px-4">
      <div className="max-w-md w-full bg-card border rounded-3xl p-8 text-center space-y-4">
        ...
        {status === "done" && (
          <p>
            Vous ne recevrez plus d'emails de MyKutub. Vous pouvez réactiver à tout moment dans vos
            paramètres.
          </p>
        )}
        {status === "need_login" && (
