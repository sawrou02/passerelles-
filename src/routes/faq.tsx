    ...
    <div className="bg-background">
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-12 md:py-16 space-y-12">
        {categories.map((cat) => (
          <section key={cat.title} className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-headline text-2xl md:text-3xl font-black text-primary">
                {cat.title}
              </h2>
            </div>
            <Accordion type="single" collapsible className="space-y-3">
              {cat.keys.map((k) => (
                <AccordionItem
                  key={k}
                  value={`${cat.title}-${k}`}
                  className="bg-card border border-border rounded-2xl px-5 md:px-6 last:border-b"
                >
                  <AccordionTrigger className="font-bold text-left hover:no-underline py-5">
                    {t(`faq.items.${k}q`)}
                  </AccordionTrigger>
        ...
        <div className="bg-gradient-to-br from-primary/10 to-secondary/10 border border-primary/20 rounded-3xl p-8 md:p-10 text-center space-y-4">
          ...
          <Link
            to="/contact"
            className="inline-flex items-center gap-2 bg-primary text-primary-foreground px-6 py-3 rounded-xl font-bold hover:opacity-90 transition"
          >
