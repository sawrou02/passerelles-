...
function SettingsPage() {
  ...
  const loadBlocked = async (uid: string) => {
    ...
    (profs ?? []).forEach((p: { id: string; display_name: string | null }) =>
      nameMap.set(p.id, p.display_name || "Utilisateur"),
    );
    setBlocked(
      rows.map((r) => ({
        id: r.id,
        blocked_id: r.blocked_id,
        name: nameMap.get(r.blocked_id) || "Utilisateur",
      })),
    );
  };
  ...
  useEffect(() => {
    ...
    supabase
      .from("profiles")
      .select(
        "notify_email, notify_sms, notify_push, phone_visible, notify_reservations, notify_messages, notify_followers, notify_admin",
      )
      .eq("id", user.id)
      .maybeSingle()
      .then(({ data }) => {
  ...
  const unblock = async () => {
    ...
    if (error) toast.error("Échec du déblocage");
      ...
      toast.success(`${confirmUnblock.name} a été débloqué`);
      loadBlocked(user.id);
    }
  ...
  const handlePwdChange = async (e: React.FormEvent) => {
    e.preventDefault();
    if (pwd.length < 6) {
      toast.error(t("settings.pwdShort"));
      return;
    }
    if (pwd !== pwd2) {
      toast.error(t("settings.pwdMismatch"));
      return;
    }
    setPwdLoading(true);
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-6">
        ...
        <section className="bg-card border rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              {
                key: "notify_reservations" as const,
                label: "Emails réservations",
                desc: "Demandes, confirmations et annulations",
              },
              {
                key: "notify_messages" as const,
                label: "Emails messages",
                desc: "Nouveau message (max 1 / 30 min)",
              },
              {
                key: "notify_followers" as const,
                label: "Emails abonnés",
                desc: "Quand quelqu'un vous suit",
              },
              {
                key: "notify_admin" as const,
                label: "Emails admin",
                desc: "Vérification, annonces officielles",
              },
            ].map((opt) => (
              <div
                key={opt.key}
              ...
              >
                ...
                <Switch
                  ...
                  onCheckedChange={(v) => savePrefs({ ...prefs, [opt.key]: v })}
                />
              </div>
            ))}
          </div>
        ...
        <section className="bg-card border rounded-2xl p-6 space-y-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
            ...
            <Switch
              checked={prefs.phone_visible}
              onCheckedChange={(v) => savePrefs({ ...prefs, phone_visible: v })}
            />
          </div>
        </section>
        ...
        <section className="bg-card border border-destructive/30 rounded-2xl p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-headline font-bold text-lg text-destructive">
              {t("settings.danger")}
            </h2>
          </div>
