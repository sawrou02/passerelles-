...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const allow = await shouldSend({
      userId: b.userId,
      emailType: `reservation_${b.kind}`,
      preferenceCol: "notify_reservations",
    });
    if (!allow.ok)
    ...
    if (b.kind === "request") {
      ...
      buttons = [
        { label: "Voir la demande", url: siteLink(b.bookId ? `/book/${b.bookId}` : "/messages") },
      ];
      ...
      buttons = [
        {
          ...
          url: siteLink(b.chatId ? `/messages/${b.chatId}` : "/messages"),
        },
      ];
    } else {
